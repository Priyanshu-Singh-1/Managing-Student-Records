...
class DbFunction{
	...
	if(false===$stmt){
		...
		if(!$rs)
		{
			echo "<script>alert('You hae entered Invalid Details')</script>";
	...
	function create_course($cshort,$cfull,$cdate){
			...
			echo "<script>alert('Enter Course Short Name')</script>";
			...
			echo "<script>alert('Enter Course Full Name')</script>";
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
   ...
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
			...
			$stmt->execute();
		   	echo "<script>alert('The student has been successfully Registered!! , your registration number is $reg.')</script>";
		 	//header('location:login.php');
...
function edit_course($cshort,$cfull,$udate,$id){
	...
	$stmt->execute();
    echo '<script>'; 
    echo 'alert("Course has been updated successfully!!")'; 
    echo '</script>';
...
function edit_subject($sub1,$sub2,$sub3,$udate,$id){
	...
	$stmt->execute();
    echo '<script>'; 
    echo 'alert("Subject has been updated successfully!!")'; 
    echo '</script>';
